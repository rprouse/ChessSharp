...
namespace Alteridem.Engine.Test
{
   ...
   public class TestMove
   {
      ...
      [TestCase( false, MoveFlags.KingCastle )]
      [TestCase( false, MoveFlags.QueenCastle )]
      [TestCase( true, MoveFlags.Invalid )]
      [TestCase( true, MoveFlags.Invalid | MoveFlags.KingCastle )]
      public void TestInvalidMove( bool invalid, MoveFlags flags )
      {
         var move = new Move( 0, 0, flags );
         Assert.AreEqual( invalid, move.Valid );
      }
   }
}
